...
void EKF_Attitude::Prior_Predict()
{
	...
	w_angular = w_angular_T.transpose();
	angular_vel = 1*angular_vel + 1*angular_acc*deltaT;

	// calculate the non-liner system, and decide two use the number of taylor expansion 
	if (using_2ndOrder)
	{
...
void EKF_Attitude::quaterinion2euler(double qw, double qx, double qy, double qz, double *Yaw, double *Pitch, double *Roll)
{
...
// //euler -> quaterinion:
// void EKF_Attitude::euler2quaterinion(double *qw, double *qx, double *qy, double *qz, double Yaw, double Pitch, double Roll)
// {
...
//	Eigen::Quaterniond quat_temp = Rotation_to_Quater(Rotation_matrix.transpose());
	...
	// Eigen::Vector3d eulerAngle = quat_temp.matrix().eulerAngles(2,1,0);
    // // cout << "yaw(z) pitch(y) roll(x) = " << eulerAngle4.transpose() << endl;

	// // quaterinion2euler(quat_temp.w(),
	// // 				  quat_temp.x(),
	// // 				  quat_temp.y(),
	// // 				  quat_temp.z(),
	// // 				  &yaw, &pitch, &roll);
	...
	// cout << quat_temp.x() << quat_temp.y() << quat_temp.z() << quat_temp.w();
