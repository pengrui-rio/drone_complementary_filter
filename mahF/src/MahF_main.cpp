...
int main(int argc, char **argv)
{
    ...
    IMU::Mahony_Attitude Mahony(Eigen::Vector2d(1.0, 0), 0);
    ...
    while(ros::ok())
    {
        if(imu_data_good && mag_data_good)
        {
            ...
            Mahony.deltaT = double(now.toSec() - last_time.toSec() );
            ...
            Mahony.Read_SensorData(measurement);

			Mahony.Mahony_Estimate();

            Eigen::Quaterniond q = Mahony.quaternion.back();
